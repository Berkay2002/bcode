...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "BCode (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "BCode (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                BCODE_DESKTOP_SKIP_BUILD: "true",
                BCODE_DESKTOP_KEEP_STAGE: "true",
                BCODE_DESKTOP_SIGNED: "true",
                BCODE_DESKTOP_VERBOSE: "true",
                BCODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
