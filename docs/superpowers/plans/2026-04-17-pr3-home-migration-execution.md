...
> Former working name: `atomic-finding-reef`.
...
| Task 3.7 inventory — `uiStateStore.ts`         | lines 4–11                          | lines 4–15 also contain the legacy-read list. Flip the primary `PERSISTED_STATE_KEY` and the `t3code:renderer-state:v3–v8` entries (storage migration copies those over before the store hydrates), but keep the older `codething:*` entries untouched — those predate the T3 rename and migration doesn't affect them. |
