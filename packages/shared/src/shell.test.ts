...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__BCODE_ENV_PATH_START__\n/a:/b\n__BCODE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__BCODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__BCODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__BCODE_ENV_PATH_END__",
        "__BCODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__BCODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__BCODE_ENV_CUSTOM_VAR_START__", "  padded value  ", "__BCODE_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
